...
class LineChartManager : OnChartValueSelectedListener {

    private val TAG: String = "LineChartManager"
    private var mLineChart: LineChart? = null
    private var mLineDataSet: ILineDataSet? = null

    constructor(lineChart: LineChart?, lineDataSet: ILineDataSet?) {
    ...
    fun initLineChart() {
        /***********************通过偏移量控制大小**********************/
        mLineChart?.setExtraOffsets(0f, 0f, 0f, 10f)
        /***********************通过偏移量控制大小***********************/
        ...
        mLineChart?.isDoubleTapToZoomEnabled = true
        // 设置能否拖拽高亮线(数据点和坐标的提示线) 默认为true
        mLineChart?.isHighlightPerDragEnabled = true
        // 设置拖拽滚动时，手放开是否会持续滚动 默认为true有持续效果 false会立即停止
        mLineChart?.isDragDecelerationEnabled = true
        // 设置持续滚动的速度快慢 0代表立即停止 1代表持续滚动速度快
        mLineChart?.dragDecelerationFrictionCoef = 0.5f
        /***********************缩放控制**********************/

        /***********************默认数据***********************/
        mLineChart?.setNoDataText("暂无数据哦")
        ...
        // 获取左侧y轴
        val yLAxis: YAxis? = mLineChart?.axisLeft
        // 设置左侧y轴是否显示 默认为true
        ...
        // 获取右侧y轴
        val yRAxis: YAxis? = mLineChart?.axisRight
        // 设置右侧y轴是否显示 默认为true
        ...
        val lineData = LineData(mLineDataSet)
        /***********************条目***********************/
        // 设置条目是否显示数值
